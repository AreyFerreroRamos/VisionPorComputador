function precision=ClassificarImatgesVersio3(name_folder, name_check_file)
    ...
    for num_image=3:length(folder)                                              % Es comença desde el tercer arxiu del directori, perquè el primer arxiu és el propi directori i el segon és el seu directori pare.
        ...
        ima_bin=~imbinarize(ima_equal, 'adaptive', 'Sensitivity', 1.0);         % Es binaritza la imatge utilitzant un llindar local de 1'0.
        se=strel('diamond', 2);                                                 % El diamant, el disc o l'esfera son igual de vàlids com a elements estructurants.
        ima_opened=imdilate(imerode(ima_bin, se), se);                          % S'ha d'aplicar un 'opening' per eliminar soroll i, sobre tot, per separar els cucs.  
        worms=bwareafilt(ima_opened, [200 3000]);                               % S'extreuen tots aquells objectes de la imatge que tinguin un tamany semblant al que s'espera que tinguin els cucs. Amb això s'aconsegueix eliminar soroll i el marc.
